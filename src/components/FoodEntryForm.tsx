...
function getBadgeColor(type: string) {
  if (type === "Maize") return "bg-yellow-100 text-yellow-800 border-yellow-400";
...
export default function FoodEntryForm() {
  ...
  function getCurrentBalance(type: string) {
    let sum = 0;
    foodRecords.forEach((rec: { foodType: string; movementType: string; quantity: number; }) => {
  ...
  const filteredRecords = foodRecords
    .filter((rec: { foodType: string; }) => filterType === "All" || rec.foodType === filterType)
  ...
  function handleUndo(idx: number) {
    ...
    setFoodRecords(foodRecords.filter(
      (rec: { date: any; foodType: any; quantity: any; movementType: any; source: any; purpose: any; notes: any; }, i: number, arr: string | any[]) => !(arr.length - 1 - i === idx && 
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {FOOD_TYPES.map(type => (
          <motion.div
            key={type}
            layout
            animate={changedTypes.includes(type)
              ? { scale: 1.09, backgroundColor: "#fde68a" }
              : { scale: 1, backgroundColor: "#fff" }
            }
            transition={{ type: "spring", stiffness: 350, damping: 22 }}
            className={`inline-flex items-center px-5 py-2 rounded-full border font-semibold shadow text-base md:text-lg ${getBadgeColor(type)}`}
                ...
                {rows.map((row, idx) => (
                  <tr key={idx}>
                    <td className="px-1 sm:px-2 py-1">
                      <select
                        className={`border rounded-lg px-2 py-1 w-full md:w-32 ${errors[idx]?.foodType && "border-red-500"
                          }`}
                        value={row.foodType}
                        onChange={e =>
                    ...
                    <td className="px-1 sm:px-2 py-1">
                      <input
                        type="number"
                        min={1}
                        className={`border rounded-lg px-2 py-1 w-full md:w-24 ${errors[idx]?.quantity && "border-red-500"
                          }`}
                        placeholder="e.g. 100"
                        value={row.quantity}
                        onChange={e =>
                    ...
                    <td className="px-1 sm:px-2 py-1">
                      {movementType === "incoming" ? (
                        <select
                          className={`border rounded-lg px-2 py-1 w-full md:w-28 ${errors[idx]?.source && "border-red-500"
                            }`}
                          value={row.source}
                          onChange={e =>
                        ...
                        <select
                          className={`border rounded-lg px-2 py-1 w-full md:w-28 ${errors[idx]?.purpose && "border-red-500"
                            }`}
                          value={row.purpose}
                          onChange={e =>
          ...
          <tbody>
            <AnimatePresence>
              {filteredRecords.map(
                (
                  rec: {
                    date: string | number | bigint | boolean | React.ReactElement<any, any> | Iterable<React.ReactNode> | React.ReactPortal | Promise<any> | null | undefined;
                    foodType: string | number | bigint | boolean | React.ReactElement<any, any> | Iterable<React.ReactNode> | React.ReactPortal | Promise<any> | null | undefined;
                    movementType: string;
                    quantity: string | number | bigint | boolean | React.ReactElement<any, any> | Iterable<React.ReactNode> | React.ReactPortal | Promise<any> | null | undefined;
                    source: string | number | bigint | boolean | React.ReactElement<any, any> | Iterable<React.ReactNode> | React.ReactPortal | Promise<any> | null | undefined;
                    purpose: string | number | bigint | boolean | React.ReactElement<any, any> | Iterable<React.ReactNode> | React.ReactPortal | Promise<any> | null | undefined;
                    notes: string | number | bigint | boolean | React.ReactElement<any, any> | Iterable<React.ReactNode> | React.ReactPortal | Promise<any> | null | undefined;
                  },
                  idx: number
                ) => (
                <motion.tr
                ...
                >
                  ...
                  <td className={`px-3 py-1 ${rec.movementType === "incoming" ? "text-green-700" : "text-red-700"}`}>
                    {rec.movementType === "incoming" ? "Incoming" : "Outgoing"}
                  </td>
                  <td className="px-3 py-1">{rec.quantity}</td>
                  <td className="px-3 py-1">{rec.movementType === "incoming" ? rec.source : rec.purpose}</td>
